                    ...
                    <table class="table table-bordered table-responsive table-striped table-hover">
                        ...
                        <tbody>
                            <tr v-for="(movie, index) in movies" :key="movie.id">
                                <td>{{index + 1}}</td>
                                <td>{{movie.name}}</td>
                                <td><img :src="movie.image" :alt="movie.name" height="50" /></td>
                                <td>{{movie.description}}</td>
                                <td>{{new Date(movie.created_at).toLocaleString() }}</td>
                                <td>
                                    <button @click="createForm(movie)" class="btn mx-2 btn-sm btn-outline-info">edit</button>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{create ? 'Add Movie' : 'Edit Movie'}}</h5>
